...
export const DashboardHeader = ({ pageTitle, branchName }: DashboardHeaderProps) => {
    ...
    <header className="sticky top-0 z-50 flex h-16 shrink-0 items-center gap-2 border-b border-primary dark:bg-sidebar px-4 backdrop-blur-xl">
      ...
      <div className="flex items-center justify-between w-full gap-4">
        <div className="min-w-0 flex-1">
          <h1 className="text-foreground font-semibold text-lg truncate">{pageTitle}</h1>
          <span className="text-foreground text-sm truncate block">{branchName}</span>
        </div>
